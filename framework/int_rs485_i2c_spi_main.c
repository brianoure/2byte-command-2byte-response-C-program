...
//##RS485##
//##SPI##
int COMMAND_RESULT1_SPI = 0;
int COMMAND_RESULT2_SPI = 0;
int COMMANDARRAY_SPI   [16];
int RESPONSEARRAY_SPI  [16];
//##SPI##
//####################################
...
int read_binary_input_spi(){
    ...
    int result=0;
     /*framework customizable
	   //spi in......input binary value is when  two lines are exctly opposite/complementing each other...you just have to agree which set represents the ONE and which set represents the ZERO
	   if( (!HAL_GPIO_ReadPin( GPIOB, GPIO_PIN_15 )) & (!HAL_GPIO_ReadPin( GPIOD, GPIO_PIN_10 )) ){result=3;}//end00
	   if( (!HAL_GPIO_ReadPin( GPIOB, GPIO_PIN_15 )) &   HAL_GPIO_ReadPin( GPIOD, GPIO_PIN_10 )  ){result=1;}//one01
	   if(   HAL_GPIO_ReadPin( GPIOB, GPIO_PIN_15 )  & (!HAL_GPIO_ReadPin( GPIOD, GPIO_PIN_10 )) ){result=0;}//zero10
	   if(   HAL_GPIO_ReadPin( GPIOB, GPIO_PIN_15 )  &   HAL_GPIO_ReadPin( GPIOD, GPIO_PIN_10 )  ){result=2;}//pause11
...
//shift all to left, insert new bit at end
//command_leftShift_insertEnd_spi
int command_leftShift_insertEnd_spi(int insertionbit){
    for( int index=0;  index<=14; index++ ){  COMMANDARRAY_SPI[index] = COMMANDARRAY_SPI[index+1];   }//for
    COMMANDARRAY_SPI[15] = insertionbit;
return 0;  
}//command_leftShift_insertEnd_spi
...
//transmit_bit_response_spi
int transmit_bit_response_spi(int X){
    //TRUTH TABLE
    //PC10 PA15 Y(TX).....................CAUTION
    //0    0    3(end)
    //0    1    1
...
//send_response_spi
int send_response_spi(){
    for( int index=0;  index<=15; index++ ){  transmit_bit_response_spi(  RESPONSEARRAY_SPI[index]  );  }//for
return 0;
}//send_response_spi
...
//reset_command_array_spi
int reset_command_array_spi(){
    for( int index=0; index<=15;  index++ ){  COMMANDARRAY_SPI[index] = 0;  }//for
return 0;
}//reset_command_array_spi
...
//reset_response_array_spi
int reset_response_array_spi(){
    for( int index=0;  index<=15; index++ ){  RESPONSEARRAY_SPI[index] = 0;  }//for
return 0;
}//reset_response_array_spi
...
//capture_command_spi    
int captured_command_spi(){
    COMMAND_RESULT1_SPI = 0;
    for( int index=0;  index<=7;index++ ){
       COMMAND_RESULT1_SPI = COMMAND_RESULT1_SPI + ( COMMANDARRAY_SPI[ index ] * ( (int)( 1<<(7-index) ) ) ); 
    }//for
    COMMAND_RESULT2_SPI = 0;
    for( int index=8;  index<=15;index++){
       COMMAND_RESULT2_SPI = COMMAND_RESULT2_SPI + ( COMMANDARRAY_SPI[ index ] * ( (int)(1<<(15-index) ) ) ); 
    }//for
return 0;
}//captured_command_spi
...
//write_response_spi
int write_response_spi( int firstbyte, int secondbyte){
    for( int index=0;  index<=7;index++ ){
    RESPONSEARRAY_SPI[index] = (int) ( ( (int) ( firstbyte >>(7 -index) ) ) & 1 ); // & 1 eliminates all preceding bits  
    }//for
    for( int index=8;  index<=15;index++ ){
    RESPONSEARRAY_SPI[index] = (int) ( ( (int) ( secondbyte>>(15-index) ) ) & 1 ); // & 1 eliminates all preceding bits 
    }//for
    send_response_spi();
    reset_response_array_spi();
return 0;
...
//execute spi
int execute_spi(){
    if(  COMMAND_RESULT1_SPI==PING ){ write_response_spi(ACK,0); }//ACK...........Fault reporting mechanisms?
    if ( COMMAND_RESULT1_SPI==SON ){
                             int else_check=1;
                             if(COMMAND_RESULT2_SPI==PL5V_EN    ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==ADCS5V_EN  ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==RS12V_EN   ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==XB12V_EN   ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==RS3V3_EN   ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==PL_EN      ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==ADCS_EN    ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==UHF_EN     ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==GPS_EN     ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==ADCS12V_EN ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(else_check==1                     ){write_response_spi(NACK,0);}//NACK
    }//SON
    if ( COMMAND_RESULT1_SPI==SOF ){
                             int else_check=1;
                             if(COMMAND_RESULT2_SPI==PL5V_EN    ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==ADCS5V_EN  ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==RS12V_EN   ){else_check=0;CURRENTMODE=CUSTOM;write_response_spi(ACK,0);     }//ACK.... do action
    ...
    if (  COMMAND_RESULT1_SPI==SM ){
                             int else_check=1;
                             if(COMMAND_RESULT2_SPI==INITIALIZE    ){else_check=0;CURRENTMODE=INITIALIZE   ;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==DETUMBLE      ){else_check=0;CURRENTMODE=DETUMBLE     ;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==NORMAL        ){else_check=0;CURRENTMODE=NORMAL       ;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==COMMUNICATION ){else_check=0;CURRENTMODE=COMMUNICATION;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==PAYLOAD       ){else_check=0;CURRENTMODE=PAYLOAD      ;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==IMAGE         ){else_check=0;CURRENTMODE=IMAGE        ;write_response_spi(ACK,0);     }//ACK.... do action
                             if(COMMAND_RESULT2_SPI==EMERGENCY     ){else_check=0;CURRENTMODE=EMERGENCY    ;write_response_spi(ACK,0);     }//ACK.... do action
                             if(else_check==1                        ){write_response_spi(NACK,0);}//NACK
    }//SM
    if (  COMMAND_RESULT1_SPI==GM    ){ write_response_spi(ACK,CURRENTMODE       ); }//ACK //GM
    if (  COMMAND_RESULT1_SPI==GSC   ){ write_response_spi(ACK,CURRENTSYSTEMCLOCK); }//ACK.........MIGHT have to do away with 2 byte response limitation OR i can just specify what each count(1) represents as a time period for a 1 byte maximum
    if (  COMMAND_RESULT1_SPI==SSC   ){ write_response_spi(ACK, 0  );CURRENTSYSTEMCLOCK=COMMAND_RESULT2_SPI; }//ACK
    if (  COMMAND_RESULT1_SPI==GOSTM ){
                             int a = (int) (XB12V_I  ()<<7);  int b = (int) (ADCS12V_I()<<6); int c = (int) (RS5V_I   ()<<5);  int d = (int) (RS3V3_I  ()<<4);
                             int e = (int) (SA1_I    ()<<3);  int f = (int) (SA2_I    ()<<2); int g = (int) (SA3_I    ()<<1);
                             write_response_rs485(  ACK, (int) (a | b | c | d | e | f | g |  1)    );
    }//ACK //GOSTM
    if ( COMMAND_RESULT1_SPI==KEN   ){ write_response_spi(ACK,KEN ); }//ACK ...........shutting down all activity received from GCS or OBC //KEN
    if ( COMMAND_RESULT1_SPI==KDIS  ){ write_response_spi(ACK,KDIS); }//ACK //KDIS
    //YOU CAN ALSO ADD LOGIC
return 0;
}//execute
...
int reset_and_assign_command_result_integers_spi(){
    COMMAND_RESULT1_SPI = 0;//refresh
    COMMAND_RESULT2_SPI = 0;//refresh
    captured_command_spi();//extract command(command_result1) and the parameter(command_result2)
return 0;
}//reset_and_assign_command_result_integers
...
while(1){//while
  ...
  //######## SPI ##############
	...
	if ( (previous_spi==2) & (raw_input_spi==1) ){  flip_21_detected_spi=1;  }
        if ( (previous_spi==1) & (raw_input_rs485==2) ){  flip_12_detected_spi=1;  }
        if ( (previous_spi==2) & (raw_input_rs485==0) ){  flip_20_detected_spi=1;  }
        if ( (previous_spi==0) & (raw_input_rs485==2) ){  flip_02_detected_spi=1;  }
        if ( flip_21_detected_spi & flip_12_detected_spi ){ 
	   command_leftShift_insertEnd_spi(1);
	   flip_21_detected_spi=0;
           flip_12_detected_spi=0;
           flip_20_detected_spi=0;
           flip_02_detected_spi=0;
	   reset_and_assign_command_result_integers_spi();
	   execute_spi();
	}//if
        if ( flip_20_detected_spi & flip_02_detected_spi ){
           command_leftShift_insertEnd_spi(0);
           flip_21_detected_spi=0;
           flip_12_detected_spi=0;
           flip_20_detected_spi=0;
           flip_02_detected_spi=0;
